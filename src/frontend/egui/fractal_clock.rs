...
use eframe::egui::{CollapsingHeader, Color32, Frame, Painter, Pos2, Rect, Shape, Slider, Stroke, Ui, Vec2};
...
impl FractalClock {
    pub fn show(&mut self, ui: &mut Ui, seconds_since_midnight: Option<f64>) {
        if !self.paused {
            self.time = seconds_since_midnight.unwrap_or_else(|| ui.input(|input_state| input_state.time));
        ...
        Frame::popup(ui.style())
            ...
            .show(ui, |ui| {
                ...
                CollapsingHeader::new("Settings")
                    .show(ui, |ui| self.show_options(ui, seconds_since_midnight));
            });
    }

    fn show_options(&mut self, ui: &mut Ui, seconds_since_midnight: Option<f64>) {
        if seconds_since_midnight.is_some() {
            ui.label(format!(
                "Local time: {:02}:{:02}:{:02}.{:01}",
                (self.time % (24.0 * 60.0 * 60.0) / 3600.0).floor(),
                (self.time % (60.0 * 60.0) / 60.0).floor(),
                (self.time % 60.0).floor(),
                (self.time % 1.0 * 1.0e1).floor()
            ));
        } else {
